...
from app.db.models import Campaign, Company, Lead, LeadStatus, Score, ScoreHistory
from app.schemas.lead import LeadCreate, LeadDetail, LeadRead
from app.schemas.score import ScoreHistoryRead
...
def list_leads(
    ...
    sort: str = Query(default="score", pattern="^(score|created_at)$"),
    ...
    if sort == "score":
        stmt = stmt.outerjoin(Score, Score.lead_id == Lead.id).order_by(
            Score.value.desc().nulls_last(), Lead.created_at.desc()
        )
    else:
...
@router.get("/{lead_id}/score-history", response_model=List[ScoreHistoryRead])
def get_score_history(lead_id: int, db: Session = Depends(get_db)) -> List[ScoreHistory]:
    lead = db.get(Lead, lead_id)
    if lead is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Lead not found.")
    return (
        db.execute(
            select(ScoreHistory).where(ScoreHistory.lead_id == lead_id).order_by(ScoreHistory.created_at)
        )
        .scalars()
        .all()
    )